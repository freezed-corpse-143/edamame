//! About edamame popover: bean art, rotating acronym tagline, installed version, author credit,
//! and three buttons.
//!
//! It deliberately reports **no** release information of its own: it used to fetch the latest
//! release on first open, which made merely opening the page a network request and put a second
//! surface in the business of rendering release state.  `modal::UpdateModal`, which the button
//! stacks on top, is the one display site for that.
//!
//! The tagline rotation is *derived* from `opened_at.elapsed()` at render time rather than mutated
//! by a tick; [`Modal::next_deadline`] tells the run loop when to wake and redraw.
...
/// Indices into `buttons`, named so the order and the `resolve` arms can't drift apart.  Every
/// button has a name: the old catch-all `ButtonPressed(_)` arm meant a fourth would have silently
/// opened GitHub.
...
impl AboutModal {
    pub(crate) fn new() -> Self {
        // Vary the opening tagline without a rand dependency: sub-second wall-clock nanos are
        // effectively uniform across user-initiated opens.
        ...
        Self {
            ...
            buttons: vec![
                // First: it sits under the installed-version row it describes, and unlike the
    ...
    /// Map a response to an outcome, shared by the key and click paths.  Every button keeps the
    /// page open, so the user returns to it from the browser or the stacked modal.
    fn resolve(&mut self, response: ModalResponse) -> ModalOutcome {
        match response {
            ...
            // A stray index acts on nothing rather than falling into whichever arm is last.
...
impl Modal for AboutModal {
    ...
    fn next_deadline(&self) -> Option<Instant> {
        // The next boundary of a `TAGLINE_INTERVAL` grid anchored at `opened_at`.  The saturating
        // conversion keeps an absurdly long session from wrapping the deadline into the past, where
        // the run loop's `> now` filter would drop it.
...
mod tests {
    ...
    fn opening_about_does_not_touch_the_network() {
        // "Check for updates" is the only thing that reaches GitHub from this page.
        let mut app = make_app();
        app.open_about_modal();
    ...
    /// Press the button at `index` on a fresh page and run what it asked the app to do.  Focus
    /// starts on button 0, so `index` Tabs reach it.
    ...
    fn the_release_notes_button_reaches_no_network() {
        // It answers out of the binary's own changelog.
        let mut app = make_app();
        app.open_about_modal();
        press_button(&mut app, RELEASE_NOTES_BUTTON);
        assert!(!app.release_check_in_flight);
    ...
    fn every_button_index_has_a_named_arm() {
        // The old catch-all at the end of `resolve` meant a new button silently opened GitHub.
        assert_eq!(AboutModal::new().buttons.len(), VIEW_ON_GITHUB_BUTTON + 1);
    }

    #[test]
    fn the_body_is_built_for_the_columns_the_frame_will_have() {
        // `body_columns` is the ceiling `ModalView` gives the body: the terminal less the minimum
        // padding.  The About body drops its pod against that number, so getting it wrong draws a
        // pod into a frame too narrow to hold it.
        assert_eq!(body_columns(Rect::new(0, 0, 80, 24)), 80 - 2 * MIN_PAD_H);
        // Narrower than the padding itself must not underflow.
