        ...
        MouseAction::Click {
                ...
                click_to_char_offset(state, col as usize, row as usize, viewport_width)
            {
                ...
                let cursor_block_is_table = state
                    ...
                    .unwrap_or(false);
                // Diagram blocks (mermaid fences, `$$...$$` math) reveal
                // as a single unit — every line of the block already
                // shows raw source — so a click that lands on another
                // line inside the same diagram block shouldn't drop drag
                // suppression and let the image flash back in for the
                // click-to-mouseup window.
